...
def carregar_receitas():
    if os.path.exists("receitas.csv"):
        with open("receitas.csv","r",newline="",encoding="utf-8") as arquivo:
            for linha in csv.DictReader(arquivo):
                receitas.append({"descricao":linha["descricao"],"valor":float(linha["valor"]),"categoria":linha["categoria"],"data":linha["data"]})

def carregar_despesas():
    if os.path.exists("despesas.csv"):
        with open("despesas.csv","r",newline="",encoding="utf-8") as arquivo:
            for linha in csv.DictReader(arquivo):
                despesas.append({"descricao":linha["descricao"],"valor":float(linha["valor"]),"categoria":linha["categoria"],"data":linha["data"]})

def carregar_orcamentos():
    if os.path.exists("orcamentos.csv"):
        with open("orcamentos.csv","r",newline="",encoding="utf-8") as arquivo:
            for linha in csv.DictReader(arquivo):
                orcamentos[linha["categoria"]]=float(linha["valor"])

def salvar_receitas():
    with open("receitas.csv","w",newline="",encoding="utf-8") as arquivo:
        e=csv.DictWriter(arquivo,fieldnames=["descricao","valor","categoria","data"])
        e.writeheader(); e.writerows(receitas)

def salvar_despesas():
    with open("despesas.csv","w",newline="",encoding="utf-8") as arquivo:
        e=csv.DictWriter(arquivo,fieldnames=["descricao","valor","categoria","data"])
        e.writeheader(); e.writerows(despesas)

def salvar_orcamentos():
    with open("orcamentos.csv","w",newline="",encoding="utf-8") as arquivo:
        e=csv.DictWriter(arquivo,fieldnames=["categoria","valor"])
        e.writeheader()
        for c,v in orcamentos.items():
            e.writerow({"categoria":c,"valor":v})
